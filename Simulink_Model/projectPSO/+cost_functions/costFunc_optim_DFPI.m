function f = costFunc_optim_DFPI(x)
...
%%%%%%%%% Fläche/Volumen
        ...
        case 'd4Out'		% Außendurchmesser Kammer/Kolben 4
            evalin('base',['mdlPara.d4Out = ', num2str(x(i-1)),'* conv.mm2m;']);
...
%%%%%%%%% Feder
        ...
        case 'xf'			% Federvorspannung [m]
			...
			switch thisVariant{1}(end-1:end)
				...
				otherwise
					if contains(thisVariant,'DFPI')||contains(thisVariant,'DA')
						% in normal case, DA variants all are NO
						evalin('base','mdlPara.xfZero = mdlPara.xf;');
					else
						error('pso:noVariant','No variant NC/NO or DA found.')
					end
			end
...
switch thisVariant{1}(end-1:end)
	...
	otherwise
		if contains(thisVariant,'DFPI')||contains(thisVariant,'DA')
			...
			error('err:initNoVar','Variants can only be NC or NO (or DA).')
		end
end
...
% Anfangsvolumen Kammer4 [m^3] 
evalin('base','mdlPara.V04 = mdlPara.Vtot4 + mdlPara.A4 *mdlPara.x04;');
