function f = costFunc_optim_Samson_3277(x)
% Stand: 30.01.2018, Editor: Maximilian Nitsch (MMNT)
...
for i = 2:size(param4Opt,1)
    switch param4Opt{i,2}
        ...
        case 'd2ch'		% Außendurchmesser Kammer/Kolben 2
...
% Anfangsvolumen Kammer [m^3] 
evalin('base','mdlPara.V02 = mdlPara.Vtot2 + mdlPara.A2 * mdlPara.x02;');

% Zylinderform forcieren (keine Trapezform wie beim DFPM)
evalin('base','mdlPara.d2ch = mdlPara.dmem;');

% Federkraft in Datenblatt gegeben -> c aus xf berechnen
evalin('base','mdlPara.c = 2.52e3/mdlPara.xf;');
...
try
    ...
    ePa = ePa ./ 5.5e5; % Normierung auf maximalen Druckfehler
